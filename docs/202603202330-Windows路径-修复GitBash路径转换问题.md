...
### 方案 1: 使用 bash -c 包装命令 ✅ (推荐)

将整个命令用引号包裹，并通过 `bash -c` 执行：

```bash
# ❌ 错误方式
docker exec kafka /opt/kafka/bin/kafka-topics.sh --list --bootstrap-server localhost:9092

# ✅ 正确方式 (注意：实际路径是 /usr/bin/kafka-topics，没有 .sh 后缀)
docker exec kafka bash -c "kafka-topics --list --bootstrap-server localhost:9092"
```

**原理**:
- `bash -c "command"` 将整个命令作为字符串传递
- Git Bash 不会转换字符串内部的路径
- 命令在容器内的 bash 中执行，路径保持不变
...
# 修复后 (正确的命令名称，在 /usr/bin/ 目录下)
docker exec kafka bash -c "kafka-topics \
    --create \
    --topic crypto_ticker \
    --bootstrap-server localhost:9092 \
    --partitions 4 \
