 ...
 * 
 * Interval Join 与 Window Join 对比：
 * 1) Interval Join：基于事件时间，为同 key 的两条流设置时间上下界进行匹配，
 *    不需要窗口触发器；优势是状态更小、延迟更低、匹配更精细；
 *    劣势是依赖水位线与事件时间正确性，且只做关联不做聚合，
 *    在高并发/乱序严重时状态清理更敏感。
 * 2) Window Join：先把两条流放入固定/滑动窗口，再在窗口内匹配；
 *    优势是窗口语义清晰、可与聚合联动；
 *    劣势是窗口等待带来更高延迟，窗口期内状态占用更大。
 * 
 * Interval Join 具体流程（示例）：
 * 1) 现货流与合约流先按 symbol 分区（keyBy），保证同一交易对进入同一算子实例。
 * 2) 设置时间区间 between(-10s, +10s)，表示允许两条流事件时间相差 ±10 秒。
 * 3) 当现货流事件到达时，会在合约流的状态里查找时间范围内的匹配事件；
 *    反之亦然（合约流到达也会在现货状态里查找）。
 * 4) 匹配成功后触发 process 计算套利机会，否则只缓存等待或过期清理。
 * 5) 多对多匹配：同一 key 下，只要时间差满足范围，现货与合约会两两匹配，
 *    可能产生多条关联结果（笛卡尔式匹配），需要在 process 中控制输出量。
 * 6) 水位线影响清理时机：事件是否“过期”由水位线推进决定，
 *    即使超过 ±10 秒范围，只要水位线未推进到上界仍可能保留在状态中等待匹配。
 * 7) 水位线来源说明：每个并行子任务（subtask）维护一条水位线，
 *    通常为“该子任务已看到的最大事件时间 - 乱序容忍”；
 *    若有多个上游输入分区，则实际生效水位线取这些输入水位线的最小值。
 * 
 * 例子：BTC-USDT
 * - 现货事件时间 10:00:05，合约事件时间 10:00:12，时间差 7 秒 → 匹配成功
 * - 现货事件时间 10:00:05，合约事件时间 10:00:30，时间差 25 秒 → 不匹配
 * 
